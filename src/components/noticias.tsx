      ...
      <section className="px-2 container">
        ...
        <div className="w-full grid grid-cols-1 place-items-center md:grid-cols-3">
          {data.map((post) => (
            <article
              key={post.id}
              className="max-w-md bg-white shadow-md overflow-hidden md:max-w-2xl flex flex-col h-full justify-between mx-5 my-2 p-2"
            >
              <div>
                <Image
                  src={post.Image}
              ...
              <div className="w-full flex items-center justify-center">
                <Link
                  href={`/${post.id}`}
                  className="text-blue-500 border-2 border-blue-500 rounded p-2 hover:bg-blue-500 hover:text-white"
                >
                  Leia mais
                </Link>
              </div>
